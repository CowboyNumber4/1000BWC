...
const Game: React.FC = () => {
  ...
  const [additionalDeckMenuVisible, setAdditionalDeckMenuVisible] =
    useState(false); // State to control additional deck menu visibility
  const [selectedAdditionalDeck, setSelectedAdditionalDeck] = useState<
    string | null
  >(null); // State to store the selected additional deck
  ...
  const handleCreateScore = async () => {
    ...
    const defaultValue = parseInt(
      prompt("Enter the default value for the new score:") || "",
      10
    );
    ...
    if (scoreName && !isNaN(defaultValue)) {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_URL}/create-score`,
          { roomName, scoreName, defaultValue }
        );
        if (response.data.success) {
          setScores((prevScores: any) => ({
            ...prevScores,
            [scoreName]: prevScores.Points.map((player: any) => ({
              name: player.name,
              value: defaultValue,
            })),
          }));
          if (socket) {
            socket.emit("updateScores", {
              ...
              scores: {
                ...scores,
                [scoreName]: scores.Points.map((player: any) => ({
                  name: player.name,
                  value: defaultValue,
                })),
              },
  ...
  const handleDeleteScore = async (scoreName: string) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/delete-score`,
        { roomName, scoreName }
      );
      if (response.data.success) {
        setScores((prevScores: any) => {
  ...
  const loadCardData = async (cardUrl: string) => {
    try {
      const response = await axios.get(
        cardUrl.replace(/\.(png|jpg|jpeg|PNG|JPG|JPEG)$/, ".json")
      );
  ...
  useEffect(() => {
    ...
    newSocket.on("updateHands", async () => {
      ...
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/lobbies/${roomName}/hands/${username}`
        );
        const handUrls = response.data.hand.map(
          (card: string) => `${import.meta.env.VITE_API_URL}${card}`
        );
    ...
    const fetchHandCards = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/lobbies/${roomName}/hands/${username}.json`
        );
    ...
    const fetchPlayers = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/lobbies/${roomName}/players`
        );
        if (response.data.success) {
  ...
  const logAction = ({
    ...
    const message = victim
      ? `<b>${player}</b> ${action} <b>${victim}</b>`
      : `<b>${player}</b> ${action}`;
  ...
  const handleDeckMenuOptionClick = (option: string) => {
    ...
    if (socket) {
      if (option === "Draw") {
        axios
          .post(`${import.meta.env.VITE_API_URL}/peek-deck`, {
            roomName,
            deckName: "main",
            count: 1,
          })
          .then((response) => {
            if (response.data.success && response.data.cards.length > 0) {
              ...
              logAction({
                player: username,
                action: "tried to draw from an empty deck",
        ...
        axios
          ...
          .then((response) => {
            if (response.data.success && response.data.cards.length > 0) {
              ...
              logAction({
                player: username,
                action: "tried to deal from an empty deck",
                backgroundColor: "#ffcccc",
              }); // Light red background
            }
          })
        ...
        axios
          ...
          .then((response) => {
            if (response.data.success && response.data.cards.length > 0) {
              ...
              logAction({
                player: username,
                action: "tried to play from an empty deck",
                backgroundColor: "#ffcccc",
              }); // Light red background
            }
          })
        ...
        const count = parseInt(
          prompt("Enter the number of cards to view:") || "",
          10
        );
        if (count > 0) {
          axios
            ...
            .then((response) => {
              if (response.data.success) {
                ...
                logAction({
                  player: username,
                  action: `viewed <b>${count}</b> cards from the deck`,
  ...
  const handleAdditionalDeckButtonClick = async () => {
    ...
    if (!additionalDeckMenuVisible) {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/lobbies/${roomName}/decks`
        );
        const uniqueDecks = response.data.decks.filter(
          (deck: string) => deck !== "main" && deck !== "discard"
        );
  ...
  const handleAdditionalDeckMenuOptionClick = (
    deckName: string,
    option: string
  ) => {
    console.log(
      `Additional deck menu option selected: ${option} for deck: ${deckName}`
    );
        ...
        axios
          ...
          .then((response) => {
            if (response.data.success && response.data.cards.length > 0) {
              ...
              logAction({
                player: username,
                action: `tried to draw from an empty deck (${deckName})`,
                backgroundColor: "#ffcccc",
              }); // Light red background
            }
          })
        ...
        axios
          ...
          .then((response) => {
            if (response.data.success && response.data.cards.length > 0) {
              ...
              logAction({
                player: username,
                action: `tried to deal from an empty deck (${deckName})`,
                backgroundColor: "#ffcccc",
              }); // Light red background
            }
          })
        ...
        axios
          ...
          .then((response) => {
            if (response.data.success && response.data.cards.length > 0) {
              ...
              logAction({
                player: username,
                action: `tried to play from an empty deck (${deckName})`,
                backgroundColor: "#ffcccc",
              }); // Light red background
            }
          })
          ...
          axios
            ...
            .then((response) => {
              if (response.data.success) {
                ...
                logAction({
                  player: username,
                  action: `viewed <b>${count}</b> cards from the <b>${deckName}</b> deck`,
                  backgroundColor: "#e6e6fa",
                }); // Light purple background
              } else {
                alert(response.data.message);
  ...
  useEffect(() => {
    if (socket) {
      socket.on("cardDrawnToField", async (cardUrl) => {
        const flippedCardUrl =
          `${import.meta.env.VITE_API_URL}/assets/FLIPPED.png`;
        const img = new Image();
        ...
        img.onload = async () => {
          ...
          socket.emit("updateImagePositions", {
            roomName,
            images: [...images, newCard],
          }); // Ensure the server is updated with the new image
        };
      });
    }
  }, [socket, images]);
  ...
  const handlePlayCard = async () => {
    if (socket && selectedCard) {
      const cardUrl = selectedCard.startsWith("/assets/")
        ? `${import.meta.env.VITE_API_URL}${selectedCard}`
        : selectedCard;
      const img = new Image();
      img.src = cardUrl;
      img.onload = async () => {
  ...
  const handlePlayFlippedCard = async () => {
    if (socket && selectedCard) {
      ...
      img.onload = async () => {
        ...
        try {
          const response = await axios.get(
            selectedCard.replace(/\.(png|jpg|jpeg|PNG|JPG|JPEG)$/, ".json")
          );
          cardData = response.data;
        } catch (error) {
          console.error("Error loading card data:", error);
  ...
  const handleFlip = async () => {
    if (socket && selectedKonvaCard) {
      ...
      img.onload = async () => {
        ...
        if (isFlipped) {
          if (
            selectedKonvaCard.data?.description ||
            selectedKonvaCard.data?.values?.length ||
            selectedKonvaCard.data?.additional?.length
          ) {
            newCardData = {
          ...
          newCardData = {
            ...
            originalCard: selectedKonvaCard.src,
          };
        }
        const newCard = {
        ...
        const updatedImages = images.map((img) =>
          img === selectedKonvaCard ? newCard : img
        );
  ...
  const handlePlayAdditionalCard = (additionalCard: string) => {
    if (socket && selectedKonvaCard) {
      ...
      if (additionalCard.startsWith(`${import.meta.env.VITE_API_URL}`)) {
        additionalCardUrl = additionalCard;
      } else if (deckName) {
        additionalCardUrl = `${import.meta.env.VITE_API_URL}/decks/${deckName}/additional/${additionalCard}`;
      } else {
        console.error(
          "Deck name could not be determined from the main card URL."
        );
        return;
      }
      ...
      img.onload = () => {
        ...
        logAction({
          player: username,
          action: "added an additional card",
  ...
  const handleSaveEdit = async () => {
    if (socket && selectedKonvaCard) {
      ...
      setImages(
        images.map((img) => (img === selectedKonvaCard ? updatedCard : img))
      );
      socket.emit("updateImagePositions", {
        roomName,
        images: images.map((img) =>
          img === selectedKonvaCard ? updatedCard : img
        ),
      });

      try {
        await axios.post(`${import.meta.env.VITE_API_URL}/save-konva-card`, {
          roomName,
          card: updatedCard,
        });
  ...
  const handleDiceRoll = () => {
    ...
    const rollResult =
      rolls.length > 1 ? `${rolls.join(" + ")} = ${sum}` : rolls[0];
  ...
  const handleShuffleIn = async () => {
    try {
      ...
      const uniqueDecks = [
        "main",
        ...new Set(
          (response.data.decks as string[]).filter(
            (deck: string) => deck !== "main" && deck !== "discard"
          )
        ),
      ];
  ...
  const handleShuffleInDeckSelect = async (deckName: string) => {
    if (socket && selectedKonvaCard) {
      const formattedCardUrl = selectedKonvaCard.src.startsWith(
        `${import.meta.env.VITE_API_URL}`
      )
        ? selectedKonvaCard.src.replace(`${import.meta.env.VITE_API_URL}`, "")
        : selectedKonvaCard.src;
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_URL}/shuffle-in-card`,
          {
            roomName,
        ...
        if (response.data.success) {
          ...
          logAction({
            player: username,
            action: `shuffled a card into the <b>${deckName}</b> deck`,
            backgroundColor: "#ffcccc",
  ...
  const handleScoreChange = async (
    ...
    if (socket) {
      try {
        const oldValue = scores[scoreName].find(
        ...
        socket.emit("updateScores", {
          ...
          scores: {
            ...scores,
            [scoreName]: scores[scoreName].map((player: any) =>
  ...
  const defaultColumn = {
    Cell: (props: any) => (
      <EditableCell {...props} updateMyData={handleScoreChange} />
    ),
  };
  ...
  const data = React.useMemo(
    () =>
      Object.keys(scores).map((scoreName) => ({
        scoreName,
        ...(scores[scoreName]
          ? scores[scoreName].reduce((acc: any, player: any) => {
    ...
    if (scoreName && !isNaN(defaultValue)) {
      try {
        ...
        if (response.data.success) {
          ...
          logAction({
            player: username,
            action: `created a new score: <b>${scoreName}</b>`,
  ...
  const handleRemoveScore = async (scoreName: string) => {
    try {
      ...
      if (response.data.success) {
        ...
        logAction({
          player: username,
          action: `removed the score: <b>${scoreName}</b>`,
          backgroundColor: "#ffb6c1",
        }); // Pink background
      } else {
        alert(response.data.message);
  ...
  const handleViewCards = async () => {
    if (selectedPlayer) {
      ...
      if (count > 0) {
        ...
        try {
          const response = await axios.get(
            `${import.meta.env.VITE_API_URL}/lobbies/${roomName}/hands/${selectedPlayer}`
          );
          const handUrls = response.data.hand.map(
            (card: string) => `${import.meta.env.VITE_API_URL}${card}`
          );
          ...
          logAction({
            player: username,
            action: `is viewing up to <b>${count}</b> cards from <b>${selectedPlayer}</b>`,
            backgroundColor: "#e6e6fa",
          }); // Light purple background
        } catch (error) {
  ...
  const handleStealCards = async () => {
    if (selectedPlayer) {
      try {
        ...
        logAction({
          player: username,
          action: `is attempting to steal a card from <b>${selectedPlayer}</b>`,
    ...
    if (socket && selectedKonvaCard) {
      try {
        const formattedCardUrl = selectedKonvaCard.src.startsWith(
          ...
          : selectedKonvaCard.src;
        const response = await axios.post(
          `${import.meta.env.VITE_API_URL}/add-card-to-hand`,
          {
            roomName,
            username,
            cardUrl: formattedCardUrl,
      ...
      if (response.data.success) {
        ...
        for (const player of players) {
          const handResponse = await axios.get(
            `${import.meta.env.VITE_API_URL}/lobbies/${roomName}/hands/${player}`
          );
    ...
    <>
        ...
        {editMenuVisible && (
          <div style={styles.editMenu}>
            <button
              ...
              onClick={() => setShowAddScorePopup(true)}
            >
              Add Score
            </button>
            <button
              style={styles.editMenuItem}
              onClick={() => setShowRemoveScorePopup(true)}
            >
              Remove Score
            </button>
          </div>
      ...
      {showAddScorePopup && (
        <div style={styles.popup}>
          <h3>Add Score</h3>
          <button style={styles.popupButton} onClick={handleAddScore}>
            Add Score
          </button>
          <button
            style={styles.popupButton}
            onClick={() => setShowAddScorePopup(false)}
          >
            Cancel
          </button>
        </div>
        ...
        <div style={styles.popup}>
          ...
          {Object.keys(scores).map((scoreName, index) => (
            <button
              key={index}
              style={styles.popupButton}
              onClick={() => handleRemoveScore(scoreName)}
            >
              {scoreName}
            </button>
          ))}
          <button
            style={styles.popupButton}
            onClick={() => setShowRemoveScorePopup(false)}
          >
            Cancel
          </button>
        </div>
        ...
        <Layer>
          {images.map((img, index) => (
            <React.Fragment key={index}>
              ...
              />
              {!img.src.includes("FLIPPED.png") &&
                img.data &&
                img.data.values &&
                img.data.values.length > 0 &&
                img.data.values.map((value: number, valueIndex: number) => {
                  ...
                  const valuesInCurrentRow = Math.min(
                    valuesPerRow,
                    totalValues - rowIndex * valuesPerRow
                  );
                  let xPosition;
                  if (valuesInCurrentRow === 1) {
                    ...
                    xPosition =
                      img.x +
                      ((colIndex + 1) * img.width) / (valuesInCurrentRow + 1) +
                      50;
                  }
                  return (
                    <Text
      ...
      {isHost && (
        <button
          style={{ ...styles.settingsButton, zIndex: 2000 }}
      ...
      <button
        style={{ ...styles.diceToggleButton, zIndex: 2000 }}
        onClick={() => setShowDicePopup(!showDicePopup)}
      >
        🎲
      </button>
      <button
        style={{ ...styles.chatToggleButton, zIndex: 2000 }}
      ...
      {chatVisible && (
        <img
          src={`${import.meta.env.VITE_API_URL}/assets/COOKIE.png`}
          alt="Cookie"
          style={styles.cookieImage}
          onClick={handleCookieClick}
          onMouseEnter={(e) =>
            (e.currentTarget.style.transform =
              styles.cookieImageHover.transform || "scale(1.2)")
          }
      ...
      {showDicePopup && (
        ...
        >
          ...
          <button style={styles.popupButton} onClick={handleDiceRoll}>
            ROLL
          </button>
        </div>
      )}
      {selectedKonvaCard && popupPosition && (
        ...
        >
          {selectedKonvaCard.data?.originalCard ? (
            <button style={styles.popupButton} onClick={handleFlip}>
              FLIP
            </button>
          ) : (
            <>
              <button style={styles.popupButton} onClick={handleInspect}>
                INSPECT
              </button>
              <button style={styles.popupButton} onClick={handleEdit}>
                EDIT
              </button>
              <button style={styles.popupButton} onClick={handleFlip}>
                FLIP
              </button>
              <button style={styles.popupButton} onClick={handleAdditional}>
                ADDITIONAL
              </button>
              <button style={styles.popupButton} onClick={handleShuffleIn}>
                SHUFFLE IN
              </button>
              <button style={styles.popupButton} onClick={handleToHand}>
                TO HAND
              </button>{" "}
              {/* New button */}
              <button style={styles.popupButton} onClick={handleDiscard}>
                DISCARD
              </button>
            </>
          )}
        ...
        <div style={styles.deckListPopup}>
          ...
          <div style={styles.deckListContainer}>
            {availableDecks.map((deck, index) => (
              <button
                key={index}
                style={styles.deckListButton}
                onClick={() => handleShuffleInDeckSelect(deck)}
              >
                {deck}
              </button>
            ))}
          ...
          <button
            style={styles.popupButton}
            onClick={() => setShowDeckListPopup(false)}
          >
            Cancel
          </button>
        </div>
      ...
      {showInspectPopup && selectedKonvaCard && (
        <div style={styles.inspectPopup}>
          ...
          <button
            style={styles.popupButton}
            onClick={() => setShowInspectPopup(false)}
          >
            Close
          </button>
        </div>
      ...
      {showEditPopup && selectedKonvaCard && (
        <div style={styles.editPopup}>
          ...
          />
          <button style={styles.popupButton} onClick={handleSaveEdit}>
            Save
          </button>
          <button
            style={styles.popupButton}
      ...
      {showAdditionalPopup && selectedKonvaCard && (
        <div style={styles.additionalPopup}>
          <div style={styles.additionalCardContainer}>
            ...
            />
            {selectedKonvaCard.data?.additional?.map(
              (additionalCard: string, index: number) => {
                const mainCardUrl = selectedKonvaCard.src;
                const deckNameMatch = mainCardUrl.match(/\/decks\/([^/]+)\//);
                const deckName = deckNameMatch ? deckNameMatch[1] : null;
                const additionalCardUrl = deckName
                  ? `${import.meta.env.VITE_API_URL}/decks/${deckName}/additional/${additionalCard}`
                  : null;
      ...
      {showPeekPopup && (
        <div style={styles.peekPopup}>
          ...
          <button
            style={styles.popupButton}
            onClick={() => setShowPeekPopup(false)}
          >
            Close
          </button>
        </div>
      ...
      {deckMenuVisible && deckButtonRef.current && (
        <div
          style={{
            ...
            width: `${
              deckButtonRef.current.getBoundingClientRect().width - 5
            }px`, // Increase the width by 2 px
          }}
        >
          <button
            style={styles.deckMenuItem}
            onClick={() => handleDeckMenuOptionClick("Peek")}
          >
            Peek
          </button>
          <button
            style={styles.deckMenuItem}
            onClick={() => handleDeckMenuOptionClick("To Field")}
          >
            To Field
          </button>
          <button
            style={styles.deckMenuItem}
            onClick={() => handleDeckMenuOptionClick("Deal")}
          >
            Deal
          </button>
          <button
            style={styles.deckMenuItem}
            onClick={() => handleDeckMenuOptionClick("Draw")}
          >
            Draw
          </button>
        </div>
      ...
      {additionalDeckMenuVisible && additionalDeckButtonRef.current && (
        <div
          style={{
            ...styles.deckMenu,
            left: `${
              additionalDeckButtonRef.current.getBoundingClientRect().left
            }px`,
            width: `${
              additionalDeckButtonRef.current.getBoundingClientRect().width - 5
            }px`, // Increase the width by 2 px
          }}
        >
          {availableDecks.map((deck, index) => (
            <div key={index} style={styles.additionalDeckMenuItem}>
              <button
                style={styles.deckMenuItem}
                onClick={() => setSelectedAdditionalDeck(deck)}
              >
                {deck}
              </button>
              {selectedAdditionalDeck === deck && (
                <div style={{ ...styles.subDeckMenu }}>
                  <button
                    style={styles.deckMenuItem}
                    onClick={() =>
                      handleAdditionalDeckMenuOptionClick(deck, "Peek")
                    }
                  >
                    Peek
                  </button>
                  <button
                    style={styles.deckMenuItem}
                    onClick={() =>
                      handleAdditionalDeckMenuOptionClick(deck, "To Field")
                    }
                  >
                    To Field
                  </button>
                  <button
                    style={styles.deckMenuItem}
                    onClick={() =>
                      handleAdditionalDeckMenuOptionClick(deck, "Deal")
                    }
                  >
                    Deal
                  </button>
                  <button
                    style={styles.deckMenuItem}
                    onClick={() =>
                      handleAdditionalDeckMenuOptionClick(deck, "Draw")
                    }
                  >
                    Draw
                  </button>
          ...
          <div style={styles.additionalDeckMenuItem}>
            <button
              style={styles.deckMenuItem}
              onClick={() => setSelectedAdditionalDeck("discard")}
            >
              Discard
            </button>
            {selectedAdditionalDeck === "discard" && (
              <div style={{ ...styles.subDeckMenu }}>
                <button
                  style={styles.deckMenuItem}
                  onClick={() =>
                    handleAdditionalDeckMenuOptionClick("discard", "Peek")
                  }
                >
                  Peek
                </button>
                <button
                  style={styles.deckMenuItem}
                  onClick={() =>
                    handleAdditionalDeckMenuOptionClick("discard", "To Field")
                  }
                >
                  To Field
                </button>
                <button
                  style={styles.deckMenuItem}
                  onClick={() =>
                    handleAdditionalDeckMenuOptionClick("discard", "Deal")
                  }
                >
                  Deal
                </button>
                <button
                  style={styles.deckMenuItem}
                  onClick={() =>
                    handleAdditionalDeckMenuOptionClick("discard", "Draw")
                  }
                >
                  Draw
                </button>
              ...
              >
                ...
                {selectedCard === card && selectedCardIndex === index && (
                  <div style={styles.cardOptions}>
                    ...
                    {cardOptions.includes("PLAY FLIPPED") && (
                      <button
                        style={styles.cardOptionButton}
                        onClick={handlePlayFlippedCard}
                      >
                        PLAY FLIPPED
                      </button>
                    )}
                    {cardOptions.includes("DISCARD") && (
                      <button
                        style={styles.cardOptionButton}
      ...
      {showPlayersPopup && (
        <div style={styles.popup}>
          ...
          <button style={styles.popupButton} onClick={handleCountCards}>
            Count Cards
          </button>{" "}
          {/* New button */}
          <button
      ...
      {showPlayerActionsPopup && selectedPlayer && (
        <div style={styles.popup}>
          ...
          <button style={styles.popupButton} onClick={handleViewCards}>
            View Cards
          </button>
          <button style={styles.popupButton} onClick={handleStealCards}>
            Steal Cards
          </button>
          <button
            style={styles.popupButton}
            onClick={handleClosePlayerActionsPopup}
          >
            Close
          </button>
        </div>
        ...
        <div style={styles.stealPopup}>
          ...
          <div style={styles.stealContent}>
            {stealCards.map((card, index) => (
              <div
                key={index}
                style={styles.stealCard}
                onClick={() => handleStealCard(index)}
              >
          ...
          <button style={styles.popupButton} onClick={confirmStealCard}>
            Confirm Steal
          </button>
          <button
            style={styles.popupButton}
            onClick={() => setShowStealPopup(false)}
          >
            Cancel
          </button>
        </div>
      ...
      {showCardCountPopup && (
        <div style={styles.cardCountPopup}>
          ...
          <table style={styles.cardCountTable}>
            ...
            <tbody>
              {Object.entries(playerCardCounts).map(
                ([player, count], index) => (
                  <tr key={index}>
          ...
          <button
            style={styles.popupButton}
            onClick={() => setShowCardCountPopup(false)}
          >
            Close
          </button>
        </div>
