...
const DeckEditing: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDeckCards = async () => {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/deck-cards`,
        {
          params: { deckName },
        }
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/edit-deck`,
        formData
      );
      if (response.data.success) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/upload-cards`,
        formData
      );
      if (response.data.success) {
        alert("Cards uploaded successfully.");
        setCards([]);
        const updatedDeckCards = await axios.get(
          `${import.meta.env.VITE_API_URL}/deck-cards`,
          {
            params: { deckName },
          }
  ...
  const handleSelectCard = async (filename: string) => {
    ...
    setCardValues([]);
    const response = await axios.get(
      `${import.meta.env.VITE_API_URL}/card-info`,
      {
  ...
  const handleDeleteCard = async () => {
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/delete-card`, {
        deckName,
  ...
  const handleDeleteCardAndJson = async () => {
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}/delete-card-and-json`, {
        deckName,
        cardName: selectedCard,
      });
      setExistingCards(existingCards.filter((card) => card !== selectedCard));
  ...
  const handleDeleteAdditionalCard = async (filename: string) => {
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/remove-additional-card`,
        {
          deckName,
          cardName: selectedCard,
  ...
  const handleSaveCard = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/save-card`,
        formData
      );
      if (response.data.success) {
  ...
  return (
    <div style={styles.container}>
      {!selectedCard && <h1 style={styles.title}>Edit Deck: {deckName}</h1>}
      {!selectedCard && (
        <>
        ...
        <div style={styles.cardEditor}>
          <div style={styles.cardEditorContent}>
            ...
            <img
              src={`${
                import.meta.env.VITE_API_URL
              }/decks/${deckName}/cards/${selectedCard}`}
              alt="card"
            ...
            <div style={styles.buttonContainer}>
              <button
                type="button"
                onClick={handleDeleteCardAndJson}
            ...
            <div style={styles.additionalCardsContainer}>
              ...
              <div style={styles.additionalCardsGrid}>
                {additionalCards.map((filename) => {
                  const imageUrl = `${
                    import.meta.env.VITE_API_URL
                  }/decks/${deckName}/additional/${filename}`;
                  return (
