...
const CreateDeck: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      deckName.toLowerCase() === "main" ||
      deckName.toLowerCase() === "discard"
    ) {
      alert(
        "Deck name cannot be 'main' or 'discard'. Please choose another name."
      );
      return;
    }
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/create-deck`,
        formData
      );
