...
const LobbyScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on("updateRoom", (roomState) => {
      ...
      const currentPlayer = roomState.players.find(
        (player: any) => player.id === newSocket.id
      );
  ...
  const handleSelectDeck = async (deck: any) => {
    if (deck.isPublic) {
      if (addingToAdditionalDecks) {
        setAdditionalDecks((prevAdditionalDecks) => [
          ...prevAdditionalDecks,
          { ...deck, id: uuidv4() },
        ]);
      } else {
      ...
      if (password) {
        const response = await axios.post(
          `${import.meta.env.VITE_API_URL}/validate-usage-password`,
          {
  ...
  const handleRemoveDeck = (deckId: string) => {
    ...
    setSelectedDecks((prevSelectedDecks) =>
      prevSelectedDecks.filter((deck) => deck.id !== deckId)
    );
  };

  const handleRemoveAdditionalDeck = (deckId: string) => {
    setAdditionalDecks((prevAdditionalDecks) =>
      prevAdditionalDecks.filter((deck) => deck.id !== deckId)
    );
  };
  ...
  useEffect(() => {
    const fetchDecks = async () => {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/search-decks`,
        {
          params: { keyword: keyword.toLowerCase(), page },
        }
      );
      const decksWithInfo = await Promise.all(
        response.data.decks.map(async (deck: any) => {
          const deckInfoResponse = await axios.get(
            `${import.meta.env.VITE_API_URL}/deck-info`,
            {
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    setDragging(true);
    setDragStart({
      x: e.clientX - settingsPosition.left,
      y: e.clientY - settingsPosition.top,
    });
  };
  ...
  return (
    <div style={styles.container}>
      ...
      {isHost && (
        <div style={styles.settingsContainer} onMouseDown={handleMouseDown}>
          <label style={styles.checkboxLabel}>
            ...
            <span style={styles.tooltip}>
              does not remove cards from decks when drawn
            </span>
          </label>
          <label style={styles.checkboxLabel}>
            <input
          ...
          <p style={{ margin: 0 }}>Total Cards: {totalCards}</p>{" "}
          {/* Display total cards */}
          <div style={styles.deckList}>
            {selectedDecks.map((deck) => (
              <div key={deck.id} style={styles.deckItem}>
                ...
                <img
                  src={`${import.meta.env.VITE_API_URL}/decks/${
                    deck.deckName
                  }/important/${deck.thumbnail}`}
                  alt="thumbnail"
          ...
          <button
            ...
            onClick={handleAddAdditionalDecksClick}
          >
          ...
          <div style={styles.additionalDeckList}>
            {additionalDecks.map((deck) => (
              <div key={deck.id} style={styles.deckItem}>
                <button
                  style={styles.removeButton}
                  onClick={() => handleRemoveAdditionalDeck(deck.id)}
                >
                  X
                </button>
                <img
            ...
            {decks.map((deck, index) => (
              ...
              >
                <button
                  style={
                    deck.isPublic
                      ? styles.publicDeckButton
                      : styles.privateDeckButton
                  }
        ...
        <div style={styles.deckInfoOverlay} onClick={handlePopupClick}>
          <div style={styles.deckInfoPopup}>
            ...
            <img
              ...
              }/important/${selectedDeckInfo.thumbnail}`}
              alt="thumbnail"
