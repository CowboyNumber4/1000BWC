...
app.post(
  ...
  (req: Request, res: Response): void => {
    const { deckName, description, editPassword, isPublic, usagePassword } =
      req.body;
    ...
    if (thumbnail && !allowedImageTypes.includes(thumbnail.mimetype)) {
      res.status(400).json({
        success: false,
        message: "Invalid image type. Please upload a JPEG, PNG, or JPG image.",
      });
      return;
    }
    ...
    if (thumbnail) {
      ...
      fs.rename(tempPath, targetPath, (err) => {
        if (err) {
          res
            .status(500)
            .json({ success: false, message: "Error moving thumbnail." });
          return;
        }
      });
    }
...
app.get("/search-decks", (req: Request, res: Response) => {
  ...
  const decks = fs.readdirSync(decksDir).filter((deckName) => {
    ...
    if (fs.existsSync(deckInfoPath)) {
      ...
      return (
        deckInfo.deckName.includes(keyword) ||
        deckInfo.description.includes(keyword)
      );
    }
    return false;
  });
  ...
  const paginatedDecks = decks
    ...
    .map((deckName) => {
      ...
      return {
        ...
        description:
          deckInfo.description.length > 100
            ? deckInfo.description.substring(0, 100) + "..."
    ...
    if (req.files) {
      ...
      for (const file of files) {
        ...
        if (existingFiles.includes(fileName)) {
          res.status(400).json({
            success: false,
            message: `Duplicate file name: ${file.originalname}`,
          });
          return;
        }
        ...
        fs.writeFileSync(
          path.join(cardsDir, `${fileName}.json`),
...
app.post(
  ...
  (req: Request, res: Response) => {
    ...
    if (req.files) {
      ...
      const newAdditionalFiles = additionalFiles.map(
...
app.post("/remove-additional-card", (req: Request, res: Response) => {
  ...
  console.log("Received remove-additional-card request:", {
    deckName,
    cardName,
    additionalCardName,
  });
  ...
  const cardInfo = JSON.parse(fs.readFileSync(cardInfoPath, "utf-8"));
  cardInfo.additional = cardInfo.additional.filter(
    (name: string) => name !== additionalCardName
  );
...
app.post("/delete-card-and-json", (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error deleting card and JSON." });
  }
});

app.post("/save-konva-card", (req: Request, res: Response) => {
  const { roomName, card } = req.body;
  const konvaCardPath = path.join(
    __dirname,
    "lobbies",
    roomName,
    "konva",
    `${path.parse(card.src).name}.json`
  );

  if (!fs.existsSync(path.join(__dirname, "lobbies", roomName, "konva"))) {
    fs.mkdirSync(path.join(__dirname, "lobbies", roomName, "konva"), {
      recursive: true,
    });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error saving card data." });
  }
});
...
app.post("/shuffle-in-card", (req: Request, res: Response) => {
  ...
  const formattedCardUrl = cardUrl.startsWith(`${process.env.API_URL}`)
    ? cardUrl.replace(`${process.env.API_URL}`, "")
    : cardUrl;
  ...
  for (let i = deckData.cards.length - 1; i > 0; i--) {
    ...
    [deckData.cards[i], deckData.cards[j]] = [
      deckData.cards[j],
      deckData.cards[i],
    ];
  }
...
app.post(
  ...
  (req: Request, res: Response) => {
    ...
    if (fs.existsSync(handPath)) {
      ...
      const blankCardIndex = handData.hand.findIndex((card: string) =>
        card.includes("BLANK.png")
      );
      if (blankCardIndex > -1) {
...
app.post("/create-score", (req: Request, res: Response) => {
  ...
  if (!settings.scores[scoreName]) {
    settings.scores[scoreName] = settings.scores.Points.map((player: any) => ({
      name: player.name,
      value: defaultValue,
    }));
    ...
    res
      .status(400)
      .json({ success: false, message: "Score type already exists." });
  }
});
...
app.post("/update-score", (req: Request, res: Response) => {
  ...
  const settings = JSON.parse(fs.readFileSync(settingsPath, "utf-8"));
  const playerScore = settings.scores[scoreName].find(
...
io.on("connection", (socket) => {
  ...
  socket.on(
    ...
    ({
      ...
      let counter = 1;
      while (
        rooms[roomName].players.some(
          (player) => player.username === uniqueUsername
        )
      ) {
  ...
  async function drawCard(
    ...
    chaosMode: boolean
  ): Promise<{ card: string | null; blankCards: number }> {
    ...
    if (isMainDeck && randomIndex >= deckLength) {
      return {
        card: "/assets/BLANK.png",
        blankCards: infiniteMode ? blankCards : blankCards - 1,
      };
    }
  ...
  socket.on(
    ...
    async ({
      ...
      if (rooms[roomName]) {
        ...
        // Initialize scores
        const scores = {
          Points: rooms[roomName].players.map((player) => ({
        ...
        fs.writeFileSync(
          path.join(lobbyPath, "settings.json"),
        ...
        fs.writeFileSync(
          path.join(decksPath, "main.json"),
          JSON.stringify({ cards: mainDeck }, null, 2)
        );
        console.log(`Main deck created with ${mainDeck.length} cards`);

        // Create discard.json for the discard pile
        fs.writeFileSync(
          path.join(decksPath, "discard.json"),
        ...
        for (const deck of additionalDecks) {
          ...
          if (fs.existsSync(deckPath)) {
            ...
            let counter = 1;
            while (
              fs.existsSync(path.join(decksPath, additionalDeckFileName))
            ) {
              additionalDeckFileName = `${deck.deckName}_${counter}.json`;
              counter++;
            }
            fs.writeFileSync(
              path.join(decksPath, additionalDeckFileName),
              JSON.stringify(
                { deckName: deck.deckName, cards: additionalDeck },
                null,
                2
              )
            );
            console.log(
              `Additional deck created: ${additionalDeckFileName} with ${additionalDeck.length} cards`
            );
          } else {
            console.error(`Deck path does not exist: ${deckPath}`);
          }
        ...
        rooms[roomName].players.forEach((player) => {
          const playerHandPath = path.join(
            handsPath,
            `${player.username}.json`
          );
  ...
  socket.on("drawCard", async ({ roomName, username, deckName }) => {
    ...
    if (fs.existsSync(deckPath)) {
      ...
      const { card, blankCards: updatedBlankCards } = await drawCard(
        deckData.cards,
        blankCards,
        isMainDeck,
        isMainDeck ? false : infiniteMode,
        isMainDeck ? chaosMode : false
      );
      ...
      fs.writeFileSync(
        settingsPath,
        JSON.stringify({ ...settings, blankCards }, null, 2)
      );
      console.log(`Card drawn: ${card} by ${username} from ${deckName}`);
      logAction(
        roomName,
        `<b>${username}</b> drew a card from <b>${deckName}</b>`,
        "#ffcccc"
      ); // Red background
    } else {
  ...
  socket.on("dealCard", async ({ roomName, username, deckName }) => {
    console.log(
      `Received dealCard event from ${username} in room ${roomName} using deck ${deckName}`
    );
    const lobbyPath = path.join(__dirname, "lobbies", roomName);
    const settingsPath = path.join(lobbyPath, "settings.json");
    ...
    if (fs.existsSync(deckPath)) {
      ...
      if (Array.isArray(deckData.cards)) {
        ...
        logAction(
          roomName,
          `<b>${username}</b> dealt a card to each player`,
          "#ffcc99"
        ); // Orange background
  ...
  function removeCardFromHand(
    ...
    const cardPath = card.replace(`${process.env.API_URL}`, ""); // Ensure the card URL is consistent
    const originalCardPath = originalCard
      ? originalCard.replace(`${process.env.API_URL}`, "")
      : null;
    console.log(
      `=== HAND: Removing card from hand. Room: ${roomName}, Username: ${username}, Card: ${cardPath}, Original Card: ${originalCardPath}`
    );
    if (fs.existsSync(handPath)) {
      const handData = JSON.parse(fs.readFileSync(handPath, "utf-8"));
      console.log(
        `=== HAND: Current hand data: ${JSON.stringify(handData.hand)}`
      );
      ...
      if (cardIndex > -1) {
        ...
        console.log(
          `=== HAND: Card removed. Updated hand data: ${JSON.stringify(
            handData.hand
          )}`
        );
      } else {
  ...
  socket.on("playCard", ({ roomName, username, card, width, height, data }) => {
    const cardUrl = card.startsWith("/assets/")
      ? `${process.env.API_URL}${card}`
      : card; // Construct the card URL
    console.log(
      `=== HAND: Playing card. Room: ${roomName}, Username: ${username}, Card: ${cardUrl}, Width: ${width}, Height: ${height}`
    );
  ...
  socket.on("discardCard", ({ roomName, username, card }) => {
    ...
    if (fs.existsSync(discardPath)) {
      ...
      const formattedCardUrl = card.startsWith(`${process.env.API_URL}`)
        ? card.replace(`${process.env.API_URL}`, "")
        : card;
  ...
  socket.on("shuffleInCard", async ({ roomName, cardUrl, deckName }) => {
    try {
      const response = await axios.post(
        `${process.env.API_URL}/shuffle-in-card`,
        { roomName, cardUrl, deckName }
      );
      if (response.data.success) {
        const updatedImages = rooms[roomName].images.filter(
          (img) => img.src !== cardUrl
        );
  ...
  socket.on("drawCardToField", async ({ roomName, username, deckName }) => {
    ...
    if (fs.existsSync(deckPath)) {
      ...
      if (Array.isArray(deckData.cards)) {
        ...
        console.log(
          `Card drawn to field: ${card} by ${username} from ${deckName}`
        );

        const flippedCardUrl = `${process.env.API_URL}/assets/FLIPPED.png`;
        const cardData = { originalCard: `${process.env.API_URL}${card}` };
        const newCard = {
        ...
        logAction(
          roomName,
          `<b>${username}</b> drew a card to the field from <b>${deckName}</b>`,
          "#ffcccc"
        ); // Red background
      } else {
        console.error(`Invalid deck format: ${deckPath}`);
  ...
  socket.on("stealCard", ({ roomName, thief, victim, cardIndex }) => {
    const victimHandPath = path.join(
      __dirname,
      "lobbies",
      roomName,
      "hands",
    ...
    if (fs.existsSync(victimHandPath) && fs.existsSync(thiefHandPath)) {
      const victimHandData = JSON.parse(
        fs.readFileSync(victimHandPath, "utf-8")
      );
      ...
      if (victimHandData.hand[cardIndex]) {
        ...
        fs.writeFileSync(
          victimHandPath,
          JSON.stringify(victimHandData, null, 2)
        );
  ...
  app.get(
    "/lobbies/:roomName/hands/:username",
    (req: Request, res: Response) => {
      ...
      if (fs.existsSync(handPath)) {
        ...
        res
          .status(404)
          .json({ success: false, message: "Hand data not found." });
      }
    }
  );
  ...
  app.get("/lobbies/:roomName/decks", (req: Request, res: Response) => {
    ...
    const decks = fs
      ...
      .map((deckFile) => path.parse(deckFile).name);
  ...
  app.get("/lobbies/:roomName/players", (req: Request, res: Response) => {
    ...
    if (fs.existsSync(handsDir)) {
      try {
        ...
        res
          .status(500)
          .json({ success: false, message: "Error reading hands directory." });
      }
    } else {
